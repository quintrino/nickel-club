...
def upgrade():
    op.execute("UPDATE nickel_request SET amount = -1 * amount WHERE request_type = 'credit'; ")
...
def downgrade():
    ...
    op.add_column('nickel_request', sa.Column('request_type', postgresql.ENUM('debit', 'credit', name='nickelrequesttype'), autoincrement=False, nullable=True))
